...
impl Generator {
    ...
    pub fn hex128_from_bytes(uuid: &[u8; 24]) -> String {
        ...
        let mut res = hex::encode(&temp_uuid[0..4]);
        res.push_str(&"-".to_string());
        res.push_str(&hex::encode(&temp_uuid[4..6]));
        res.push_str(&"-".to_string());
        res.push_str(&hex::encode(&temp_uuid[6..8]));
        res.push_str(&"-".to_string());
        res.push_str(&hex::encode(&temp_uuid[8..10]));
        res.push_str(&"-".to_string());
        res.push_str(&hex::encode(&temp_uuid[10..16]));
        res
    }

    //  Returns true if provided string is a valid 128-bit UUID
    pub fn valid_hex128(uuid: &str) -> bool {
...
mod tests {
    ...
    fn hex128() {
        let generator = Generator::new();
        assert!(
            Generator::valid_hex128(&generator.hex128()[..]),
            "should be valid hex"
        );
    }
    ...
    fn valid_hex() {
        // valid v4 uuid
        assert!(
            Generator::valid_hex128("11febf98-c108-4383-bb1e-739ffcd44341"),
            "should be valid hex"
        );

        // invalid uuid
        assert!(
            !Generator::valid_hex128("11febf98-c108-4383-bb1e-739ffcd4434"),
            "should be invalid hex"
        );
        assert!(
            !Generator::valid_hex128("11febf98-c108-4383-bb1e-739ffcd443412"),
            "should be invalid hex"
        );
        assert!(
            !Generator::valid_hex128("11febf98c1-08-4383-bb1e-739ffcd44341"),
            "should be invalid hex"
        );
        assert!(
            !Generator::valid_hex128("11febf98-c1084-383-bb1e-739ffcd44341"),
            "should be invalid hex"
        );
        assert!(
            !Generator::valid_hex128("11febf98-c108-4383bb-1e-739ffcd44341"),
            "should be invalid hex"
        );
        assert!(
            !Generator::valid_hex128("11febf98-c108-4383-bb1e7-39ffcd44341"),
            "should be invalid hex"
        );
    }
}
